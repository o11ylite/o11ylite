...
(defn create-notebook!
  ...
  [sqlite id {:keys [name description global_from global_to]}]
  (let [now (-now-ms)]
    (jdbc/execute!
     sqlite
     ["INSERT INTO notebooks (id, name, description, global_from, global_to, created_at, updated_at)
        VALUES (?, ?, ?, ?, ?, ?, ?)"
      id name description
      (or global_from "now-1h") (or global_to "now")
      now now])
    id))
...
(defn update-notebook!
  ...
  (let [now (-now-ms)]
    (jdbc/execute!
     sqlite
     ["UPDATE notebooks
        SET name = ?, description = ?, global_from = ?, global_to = ?, updated_at = ?
        WHERE id = ?"
      name description global_from global_to now id])))
