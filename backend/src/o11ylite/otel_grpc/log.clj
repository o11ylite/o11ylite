...
(defn- -log-handler
  ...
  [duckdb blocked-fields batcher id-generator ^ExportLogsServiceRequest request]
...
(defn create-service
  ...
  (proxy [LogsServiceGrpc$LogsServiceImplBase] []
    (export
      ...
      (try
        (let [response-map (-log-handler duckdb blocked-fields batcher id-generator request)
              response (log-events/log-response->proto (or response-map {}))]
          (.onNext response-observer response)
          (.onCompleted response-observer))
        (catch Exception e
          (telemetry/report-error! ::log-export-error e)
