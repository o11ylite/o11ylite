...
(defn- -build-event-fields
  ...
  [event-metadata sqlite]
  (let [fields (event-metadata/get-fields event-metadata)
        blocked (blocked/get-blocked-event-fields sqlite)]
    (->> fields
...
(defn- -build-metric-attributes
  ...
  (let [attr-fields (schema/fetch-metric-attr-fields duckdb)
        blocked (blocked/get-blocked-metric-fields sqlite)]
    (mapv (fn [field-name]
            {:name field-name
...
(defn- -make-page-handler
  ...
  [{:keys [sqlite event-metadata duckdb]}]
  (fn [_request]
    (response/inertia "DataManagement"
                      {:event_fields (-build-event-fields event-metadata sqlite)
                       :metrics (-build-metrics sqlite)
                       :metric_attributes (-build-metric-attributes duckdb sqlite)})))
...
(defn- -make-event-fields-status-handler
  ...
  (fn [request]
    (let [fields (get-in request [:body :fields])
          ...
          n (count fields)]
      (case status
        "blocked" (blocked/block-event-fields! sqlite fields)
        "active" (blocked/unblock-event-fields! sqlite fields))
...
(defn- -make-event-fields-delete-handler
  ...
  [{:keys [sqlite duckdb event-metadata]}]
  (fn [request]
    (let [fields (get-in request [:body :fields])
          n (count fields)]
      ;; 1. Block first to prevent schema evolution from re-adding
      (blocked/block-event-fields! sqlite fields)
...
(defn- -make-metric-attrs-status-handler
  ...
  (fn [request]
    (let [fields (get-in request [:body :fields])
          ...
          n (count fields)]
      (case status
        "blocked" (blocked/block-metric-fields! sqlite fields)
        "active" (blocked/unblock-metric-fields! sqlite fields))
      (-flash-message (str (if (= status "blocked") "Blocked " "Activated ")
...
(defn- -make-metric-attrs-delete-handler
  ...
  (fn [request]
    (let [fields (get-in request [:body :fields])
          n (count fields)]
      ;; 1. Block first to prevent schema evolution from re-adding
      (blocked/block-metric-fields! sqlite fields)
      ;; 2. DROP COLUMN from DuckDB
